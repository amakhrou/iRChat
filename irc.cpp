...
void Server::processMessage(const std::string& message, int fd)
{
    ...
    std::string command, channel_name_t,word, text;
    iss >> command >> channel_name_t;
...
int Server::kick_memeber(std::string &channel_kicked_from, std::string &user_kicked, std::string reason, int fd)
{
    ...
    { 
        ...
        if((*it1 == user_kicked && if_member(user_kicked, channel_kicked_from) == true) || (*it1 != get_admin(channel_kicked_from) && if_member(user_kicked, channel_kicked_from) == true))
        {
